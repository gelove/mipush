...
class MiPusher
{
    public function sendToIOS($regId)
    {

        $secret = config('common.mi_push_secret_ios');
        $bundleId = config('common.mi_push_bundle_ios');
...
//        Constants::useOfficial();
        ...
        $sender = new Sender();
        print_r($sender->send($message, $regId)->getRaw());
    }

    public function sendToAndroid($regId)
    {
        $secret = config('common.mi_push_secret_and');
        $package = config('common.mi_push_bundle_and');
